...
final class RejectDocumentHandler
{
    ...
    public function __invoke(RejectDocumentCommand $command): void
    {
        ...
        $rejectedBy = $this->userRepository->findById($command->rejectedBy);
        if ($rejectedBy === null) {
            throw new \DomainException('Utilisateur introuvable.');
        }

        $document->reject($rejectedBy, $command->reason);
