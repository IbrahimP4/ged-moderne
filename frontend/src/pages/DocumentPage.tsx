  ...
  return (
    <Modal open={open} onClose={onClose} title="Rejeter le document">
      <div className="space-y-4">
        <label className="block text-sm font-medium text-primary mb-1.5">Motif du rejet</label>
        <textarea
          autoFocus
          value={reason}
          onChange={(e) => setReason(e.target.value)}
          rows={3}
          className="w-full px-3.5 py-2.5 rounded-lg border border-base text-sm focus:outline-none focus:ring-2 focus:ring-red-500 resize-none"
    ...
    <Modal open={open} onClose={onClose} title="Renommer le document">
      <div className="space-y-4">
        <input
          ...
          className="w-full px-3.5 py-2.5 rounded-lg border border-base text-sm focus:outline-none focus:ring-2 focus:ring-[#F5A800]"
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        className="flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium text-secondary bg-card border border-base hover:border-strong hover:bg-muted transition-all shadow-sm"
      >
        <MoreHorizontal size={16} />
        <span className="hidden sm:inline">Plus</span>
      </button>
      {open && (
        <div className="absolute right-0 top-full mt-1.5 w-48 bg-card rounded-lg shadow-xl border border-base py-1 z-50">
          <button
            onClick={() => { onRename(); setOpen(false) }}
            className="flex items-center gap-3 w-full px-4 py-2.5 text-sm text-primary hover:bg-muted transition-colors"
          >
            <Pencil size={15} className="text-faint" />Renommer
          </button>
          <button
            onClick={() => { onMove(); setOpen(false) }}
            className="flex items-center gap-3 w-full px-4 py-2.5 text-sm text-primary hover:bg-muted transition-colors"
          >
            <FolderOpen size={15} className="text-faint" />Déplacer
          </button>
          <div className="mx-3 my-1 border-t border-base" />
          <button
...
function StatusBadge({ status }: { status: DocumentStatus }) {
  ...
  const variants: Record<DocumentStatus, string> = {
    ...
    draft: 'bg-muted border-strong text-secondary',
    archived: 'bg-muted border-strong text-secondary',
  }
  return (
...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <h2 className="text-xs font-bold text-secondary uppercase tracking-widest">{children}</h2>
    </div>
  )
}
...
export function DocumentPage() {
  ...
  const renameMutation = useMutation({
    mutationFn: (title: string) => renameDocument(id!, title),
  ...
  const moveMutation = useMutation({
    mutationFn: (folderId: string) => moveDocument(id!, folderId),
  ...
  const tagsMutation = useMutation({
    mutationFn: (tags: string[]) => updateDocumentTags(id!, tags),
  ...
  if (!doc) return (
    <div className="flex flex-col items-center justify-center h-full py-20">
      ...
      <p className="text-secondary font-medium">Document introuvable</p>
    </div>
  )
  ...
  return (
    <div className="min-h-full bg-page">
      ...
      {/* ── Top breadcrumb bar ──────────────────────────────────────────── */}
      <div className="bg-card border-b border-base px-4 sm:px-6 py-3">
        <nav className="flex items-center gap-1.5 text-sm text-muted">
          ...
          <span className="text-primary font-medium truncate max-w-xs">{doc.title}</span>
        </nav>
      </div>
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 py-4 sm:py-6 space-y-5">
        ...
        {/* ── Hero card ─────────────────────────────────────────────────── */}
        <div className="bg-card rounded-lg border border-base shadow-sm">
          {/* Colored top stripe */}
          <div className={`h-1 w-full rounded-t-lg ${heroStripe}`} />
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div>
                    ...
                    <p className="text-sm text-faint mt-0.5">{doc.originalFilename}</p>
                  </div>
                ...
                <div className="flex items-center gap-3 mt-3 flex-wrap">
                  <span className="bg-muted text-secondary text-xs px-2.5 py-1 rounded-md">
                    {formatBytes(doc.fileSizeBytes)}
                  </span>
                  <span className="bg-muted text-secondary text-xs px-2.5 py-1 rounded-md">
                    {doc.mimeType}
                  </span>
                  <span className="text-xs text-faint">Modifié {formatDate(doc.updatedAt)}</span>
                </div>
              </div>
            </div>

            {/* Actions toolbar */}
            <div className="flex items-center gap-2 mt-5 pt-5 border-t border-muted flex-wrap">
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium border transition-all shadow-sm disabled:opacity-50 ${doc.isFavorite
                    ? 'bg-[#FFF3CC] border-[#F5D580] text-[#A07000] hover:bg-[#FFE999]'
                    : 'bg-card border-base text-secondary hover:border-strong hover:bg-muted'
                  }`}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="bg-card rounded-lg border border-base shadow-sm overflow-hidden">
              <div className="px-5 py-4 border-b border-base flex items-center gap-3">
                    ...
                    return (
                      <div key={step.key} className="flex items-center flex-1">
                        <div className="flex flex-col items-center flex-1">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all ${isRejected ? 'bg-red-500 text-white'
                              : isDone ? 'bg-[#1A1A1A] text-white'
                                : isCurrent ? 'bg-[#F5A800] text-primary ring-2 ring-[#F5A800] ring-offset-1'
                                  : 'bg-border text-faint'
                            }`}>
                            {isRejected ? <XCircle size={16} /> : isDone ? <CheckCircle2 size={16} /> : i + 1}
                          </div>
                          <span className={`text-xs mt-1.5 font-medium ${isRejected ? 'text-red-600'
                              : isDone ? 'text-primary'
                        ...
                        {i < WORKFLOW_STEPS.length - 1 && (
                          <div className={`h-0.5 flex-1 mx-1 rounded-full mb-5 ${currentStep > i && doc.status !== 'rejected'
                              ? 'bg-[#1A1A1A]'
                              : 'bg-border'
                            }`} />
                        )}
                ...
                {doc.versions && doc.versions.length > 0 ? (
                  <div className="space-y-2">
                    {[...doc.versions].reverse().map((v, i) => (
                      <div
                        key={v.id}
                        className={`flex items-center justify-between p-3.5 rounded-lg transition-colors group ${i === 0
                            ? 'bg-[#FFF8E7] border border-[#F5D580]'
                            : 'bg-subtle hover:bg-muted border border-transparent'
                          }`}
                      >
                        <div className="flex items-center gap-3">
                          <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold shrink-0 ${i === 0
                              ? 'bg-[#1A1A1A] text-white'
                              : 'bg-card border border-strong text-secondary'
                            }`}>
                            v{v.versionNumber}
                          </div>
                          <div>
                            ...
                            {v.comment && (
                              <p className="text-xs text-secondary mt-0.5 italic">{v.comment}</p>
                            )}
                          </div>
                        </div>
                        <button
                          ...
                          className="p-2 rounded-lg text-faint hover:text-[#F5A800] hover:bg-card transition-all opacity-0 group-hover:opacity-100"
          ...
          <div className="space-y-5">
            ...
            <div className="bg-card rounded-lg border border-base shadow-sm overflow-hidden">
              ...
              <div className="p-5 space-y-4">
                <div>
                  <p className="text-[10px] font-bold text-faint uppercase tracking-widest mb-1">Format</p>
                  <p className="text-sm text-primary font-medium">
                    {doc.mimeType.split('/')[1]?.toUpperCase() ?? doc.mimeType}
                  </p>
                </div>
                <div>
                  <p className="text-[10px] font-bold text-faint uppercase tracking-widest mb-1">Taille</p>
                  <p className="text-sm text-primary font-medium">{formatBytes(doc.fileSizeBytes)}</p>
                </div>
                <div>
                  <p className="text-[10px] font-bold text-faint uppercase tracking-widest mb-1">Créé le</p>
                  <p className="text-sm text-primary font-medium">{formatDate(doc.createdAt)}</p>
                </div>
                <div>
                  <p className="text-[10px] font-bold text-faint uppercase tracking-widest mb-1">Modifié le</p>
                  <p className="text-sm text-primary font-medium">{formatDate(doc.updatedAt)}</p>
                </div>
                {doc.comment && (
                  <div>
                    <p className="text-[10px] font-bold text-faint uppercase tracking-widest mb-1">Note</p>
                    <p className="text-sm text-secondary italic">"{doc.comment}"</p>
                  </div>
                )}
              </div>
            </div>
                ...
                {editingTags ? (
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {(doc.tags ?? []).map((tag) => (
                      <span
                        key={tag}
                        className="bg-muted border border-strong text-secondary text-xs px-2 py-0.5 rounded-md"
                      >
                        {tag}
                      </span>
                    ))}
                  ...
                  <button
                    ...
                    className="w-full text-center text-sm text-faint py-4 border-2 border-dashed border-base rounded-lg hover:border-[#F5A800] hover:text-[#F5A800] transition-colors"
                  >
                    + Ajouter des tags
                  </button>
                )}
